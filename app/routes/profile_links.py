...
def create_profile_link(
    ...
    new_link = ProfileLink(
        profile_id=profile_id, label=link_data.label, url=link_data.url
    )
...
def get_profile_links(
    ...
    links = (
        ...
        .order_by(ProfileLink.display_order.asc(), ProfileLink.created_at.asc())
        .all()
    )
...
def delete_profile_link(
    link_id: UUID, current_user=Depends(get_current_user), db: Session = Depends(get_db)
):
    link = validate_link_in_db(link_id, db)
...
def reorder_profile_links(
    payload: ProfileLinkReorderRequest,
    current_user=Depends(get_current_user),
    db: Session = Depends(get_db),
):
