...
def validate_profile_user(profile_id: UUID, current_user: User, db: Session):
    profile = (
        db.query(Profile)
        .filter(
            Profile.profile_id == profile_id, Profile.user_id == current_user.user_id
        )
        .first()
    )
...
def validate_card_status(status_data: CardStatusUpdate):
    valid_statuses = ["inactive", "active", "deactivated", "lost", "disabled"]
...
def validate_card_profile_user(
    card_id: UUID, profile_id: UUID, current_user: User, db: Session
):
    ...
    if valid_card.profile_id != profile.profile_id:
        raise HTTPException(
            status_code=403, detail="Card does not belong to the specified profile"
        )
