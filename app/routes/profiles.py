...
def get_my_profiles(
    ...
    profiles = (
        ...
        .order_by(Profile.display_order.asc(), Profile.created_at.asc())
        .all()
    )
...
def reorder_profiles(
    updates: ProfileOrderUpdateRequest,
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
):
    ...
    if len(profiles) != len(set(profile_ids)):
        raise HTTPException(
            status_code=400,
            detail="Profile reorder request contains duplicate profile IDs.",
        )
...
def deactivate_profile(
    ...
    db.query(Card).filter(
        Card.profile_id == profile_id, Card.card_status == CardStatus.active
...
def activate_profile(
    ...
    db.query(Card).filter(
        Card.profile_id == profile_id, Card.card_status == CardStatus.deactivated
    ).update(
        {"card_status": "active"},
...
def delete_profile(
    profile_id: UUID,
    reassign_to_profile_id: UUID | None = None,
    current_user=Depends(get_current_user),
    db: Session = Depends(get_db),
):
    if reassign_to_profile_id == profile_id:
        raise HTTPException(
            status_code=400,
            detail="Cannot reassign cards to the same profile being deleted.",
        )
    ...
    if reassign_to_profile_id:
        ...
        db.query(Card).filter(Card.profile_id == profile_id).update(
            {Card.profile_id: reassign_to_profile_id}
        )
...
def add_link_and_card_counts(profile, db):
    profile.link_count = (
        ...
        .filter(ProfileLink.profile_id == profile.profile_id)
        .count()
    )
    profile.card_count = (
        db.query(Card)
        .filter(
            Card.profile_id == profile.profile_id, Card.card_status == CardStatus.active
        )
        .count()
    )
