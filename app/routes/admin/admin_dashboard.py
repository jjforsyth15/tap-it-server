...
from app.schemas.admin.admin_dashboard import (
...
def admin_dashboard_summary(
    ...
    response = DashboardSummaryResponse(
        ...
        active_cards=db.query(Card)
        .filter(Card.card_status == CardStatus.active)
        .count(),
        inactive_cards=db.query(Card)
        ...
        open_feedback=db.query(BetaFeedback)
        .filter(BetaFeedback.feedback_status == FeedbackStatus.open)
        .count(),
        in_progress_feedback=db.query(BetaFeedback)
        .filter(BetaFeedback.feedback_status == FeedbackStatus.in_progress)
        .count(),
        resolved_feedback=db.query(BetaFeedback)
        .filter(BetaFeedback.feedback_status == FeedbackStatus.resolved)
        .count(),
    )
...
def action_items(
    ...
    return AdminActionItemsResponse(
        action_items=items, total_action_items=sum(item.count for item in items)
    )
