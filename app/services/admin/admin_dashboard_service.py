        ...
        activities.append(
            AdminRecentActivityItem(
                ...
                metadata={
                    ...
                    "profile_id": (
                        card.profile_id if card.profile_id is not None else None
                    ),
                },
            )
        )
...
def get_admin_action_items(
    ...
    pending_feedback_count = (
        db.query(func.count(BetaFeedback.feedback_id))
        .filter(BetaFeedback.feedback_status == FeedbackStatus.open)
        .scalar()
        or 0
    )
    unassigned_cards_count = (
        db.query(func.count(Card.card_id)).filter(Card.profile_id == None).scalar() or 0
    )
    lost_cards_count = (
        db.query(func.count(Card.card_id))
        .filter(Card.card_status == CardStatus.lost)
        .scalar()
        or 0
    )
    inactive_users = (
        db.query(func.count(User.user_id)).filter(User.is_active == False).scalar() or 0
    )
    ...
    action_items.sort(
        key=lambda item: (priority_order.get(item.priority, 99), -item.count)
    )
